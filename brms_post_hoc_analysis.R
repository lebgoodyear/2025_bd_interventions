...
# view heatmap to see if some variable combinations are more represented than others
# or if there are any stand out interactions 
...
plotInteraction(df, "Habitat.or.Individual", "Life.Stage", 
                labs_hab, labs_life)
plotInteraction(df, "Intervention.category.itra.multi", "Habitat.or.Individual", 
                labs_intcat, labs_hab)
plotInteraction(df, "Intervention.category.itra.multi", "Life.Stage", 
                labs_intcat, labs_life)
...
legend_labels <- c(
  ...
  "Life" = "Life stage" #Add as many as you need.
)
...
plot <- ggplot(combined_data, aes(x = .value, y = 1)) + #y=1 is a placeholder for faceting.
  ...
  xlim(x_limits)+
  scale_fill_viridis_d(labels = legend_labels)# Manual legend labels
...
calc_probabilities <- function(df, var, reflev, mod) {
    ...
    # cxtract posterior samples of fixed effects
