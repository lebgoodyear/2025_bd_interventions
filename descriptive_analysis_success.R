...
# remove single translocation intervention
#df_plot <- df[df$Intervention.category.1.itra != "Translocation",]

# create table for stacked bar
...
# ggplot(data=tab_stacked, aes(Habitat.or.Individual, y=Count, fill=Intervention.category.1.itra)) +
#   geom_bar(
#     position = position_stack(reverse = TRUE),
#     stat = "identity",
#     colour = "black",
...
#     width = 0.8
#   ) + 
#   scale_fill_manual(values=colours_intcat, labels=intcat_labs) +
#   scale_x_discrete(labels = habindv_labs) +
#   labs(
#     x = "Habitat or Indiviudual",
#     y = "Count",
#     fill = "Intervention category",
#   ) +
#   theme_bw() +
#   theme(
#     panel.grid.major.y = element_blank(),  # Remove vertical gridlines
...
# plot
ggplot(data = df_cum_pubyear, aes(x=Year.of.publication, y=CumCount, colour=Intervention.category.itra.multi)) +
  geom_jitter() +
  scale_colour_manual(values=colours_intcat) +
  theme_bw()
